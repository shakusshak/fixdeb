...
// ruleIDs extracts the protovalidate rule ids (e.g. "string.max_len") from a
// validation error. Since protovalidate v1.1+ the rule id is no longer part of
// ValidationError.Error(), so tests must inspect the violations directly.
func ruleIDs(t *testing.T, err error) []string {
	t.Helper()
	var valErr *protovalidate.ValidationError
	require.ErrorAs(t, err, &valErr, "expected a *protovalidate.ValidationError")
	ids := make([]string, 0, len(valErr.Violations))
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectValid {
				...
				if tt.expectedErr != "" {
					assert.Contains(t, ruleIDs(t, err), tt.expectedErr, "Expected violation with rule id '%s', got %q", tt.expectedErr, err.Error())
				}
			}
		})
	}
